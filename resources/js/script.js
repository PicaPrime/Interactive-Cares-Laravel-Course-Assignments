document.addEventListener("DOMContentLoaded", () => {
    ...
    const mobileDropdownSubToggles = document.querySelectorAll(
        ".mobile-dropdown-sub-toggle"
    );
    ...
    mobileDropdownSubToggles.forEach((toggle) => {
        toggle.addEventListener("click", function (e) {
            ...
            if (parentMenu) {
                parentMenu
                    .querySelectorAll(".mobile-dropdown-submenu")
    ...
    document.addEventListener("click", (e) => {
        if (
            ...
            document
                .querySelectorAll(
                    ".mobile-dropdown-menu, .mobile-dropdown-submenu"
                )
                .forEach((menu) => {
    ...
    document
        .querySelector('.avatar-upload input[type="file"]')
        .addEventListener("change", function (e) {
            const file = e.target.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = function (event) {
                    document.querySelector(".avatar-upload img").src =
                        event.target.result;
                };
    ...
    tabs.forEach((tab) => {
        tab.addEventListener("click", function (e) {
            e.preventDefault();
            tabs.forEach((t) =>
                t.classList.remove("tab-active", "text-blue-600", "font-medium")
            );
...
// Add this to your JS file (e.g., resources/js/script.js)
...
// Handles desktop sub-dropdowns
document.querySelectorAll(".desktop-dropdown-sub").forEach((sub) => {
    const button = sub.querySelector("button");
    const submenu = sub.querySelector(".desktop-dropdown-submenu");
    if (button && submenu) {
